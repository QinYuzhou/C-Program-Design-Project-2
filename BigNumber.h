...
struct Big_number
{
    ...
    Big_number operator+(Big_number number)const; //进行加法计算
    
    Big_number operator-(Big_number number)const; //进行减法计算,a-b就是a+(-b)

    Big_number operator*(Big_number number)const; //进行乘法计算
    
    Big_number half(); //返回该数除2的结果(除2就是乘0.5)
   
    Big_number operator/(Big_number number)const; //进行除法计算 (a*10^k)/(b*10^m)=(a/b)*10^(k-m),通过二分答案实现
